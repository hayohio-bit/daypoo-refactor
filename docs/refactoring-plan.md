...
- [ ] **동작하지 않는 촬영 UI 제거** — AI 모듈 제거 후 남은 잔재다. `VisitModal.tsx:260-262`가 "AI 간편 촬영 분석 / 상태를 촬영하면 AI가 자동으로 분석해드립니다"라고 안내하고 카메라 촬영 UI를 제공하지만, 촬영본이 도달하는 백엔드에는 이를 읽는 코드가 없다(`service/` 전체에 `imageBase64` 참조 0곳). 제거 대상: 프론트는 `VisitModal.tsx`의 카메라 상태·`startCamera`/`stopCamera`·촬영 UI 블록(269-332행)과 사진 없이 진행하는 확인 모달, `MapPage.tsx:359`의 페이로드 조합, `types/api.ts`의 `imageBase64` 필드와 AI 관련 주석. 백엔드는 `PooRecordCreateRequest.imageBase64` 필드와 참조 0곳인 고아 DTO `AiAnalysisRequest`·`PooAnalysisRequest`. 문서는 README 아키텍처 다이어그램의 `WebRTC[무음 Canvas 촬영]` 노드와 기술 스택표의 "WebRTC 기반 무음 캡처" 표기. **착수 전 확인**: 요청 DTO에서 필드를 빼면 구버전 클라이언트가 보내는 `imageBase64`를 백엔드가 어떻게 처리하는지(Jackson 미지의 필드 정책) 검증해야 한다.
