...
- **핵심 가치:** 화장실 탐색(급똥 대응) + 방문 인증(기록) + 건강 리포트 + 랭킹. (AI 분석과 상점·칭호는 원본 팀 프로젝트의 계획 사양이며 현재 저장소에는 없다. 4.3 절과 3.1 절 참고)
- **기본 설계 원칙:**
...
> **제거됨:** 칭호(Title)·상점 아이템(Item)·구독(Subscription) 모델과 `User.title_id`·`avatar_id` 는 코드에서 제거되었다. 과거 데이터 보존을 위해 DB 테이블만 유지하며, 회원 탈퇴 시 `UserDeletionService` 가 잔여 행을 정리한다.
...
- **Health Report:** 누적된 배변 기록을 기반으로 주간·월간 단위의 통계를 계산하여 리포트를 생성한다(`ReportService`).
- **미구현 계획:** 이미지를 분석하여 브리스톨 척도와 이상 징후를 자동으로 감지하는 기능은 원본 팀 프로젝트의 계획 사양이며, 현재 저장소에는 구현되어 있지 않다.
