...
function ConicGlow({
  ...
  thickness = 1.5,
  borderRadius = '16px',
}: { color: string; thickness?: number; borderRadius?: string }) {
  return (
...
const TAB_CONFIG: { key: TabKey; label: string; desc: string; icon: React.ReactNode }[] = [
  {
    key: 'total',
    label: '전체 랭킹',
    desc: '기록 인증 + 장소 방문 점수 합산',
    icon: <Trophy size={16} />,
  },
  {
    key: 'local',
    label: '우리 동네 왕',
    desc: '현재 위치 기반 지역 활동 점수',
    icon: <MapPin size={16} />,
  },
...
function ItemPopup({
  ...
  onClose,
  openAuth,
}: { user: RankUser; onClose: () => void; openAuth: (mode: 'login' | 'signup') => void }) {
...
const FlipGlassCard = ({
  ...
  >
    ...
    <div
      className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full font-black text-[10px] shadow-xl z-[60] transition-transform duration-500"
      style={{
    ...
    >
      {user.rank}
      {user.rank === 1 ? 'ST' : user.rank === 2 ? 'ND' : 'RD'}
    </div>
    ...
    >
      ...
      >
        ...
        <div className="relative mb-3 sm:mb-5 mt-2 sm:mt-4">
          ...
          {isFirst && (
            <div className="absolute -top-5 -right-2 text-amber-500 drop-shadow-lg scale-110">
              <Crown size={24} />
            </div>
          )}
        </div>

        <h3 className="font-black text-[#1A2B27] text-[11px] sm:text-[16px] mb-0.5 sm:mb-1 truncate w-full text-center">
          {user.nick}
        </h3>
        <p className="font-black text-sm sm:text-xl" style={{ color: '#52b788' }}>
          {user.score.toLocaleString()}
          <span className="text-[8px] sm:text-[10px] uppercase font-bold text-gray-400 ml-0.5">
            {user.scoreLabel}
          </span>
        </p>
      </div>
      ...
      >
        ...
        <div className="relative mb-4">
          <div className="text-[9px] font-black text-amber-500/60 uppercase tracking-[0.3em] mb-2 text-center">
            Power Status
          </div>
        ...
        <div className="w-full space-y-4">
          <div className="flex flex-col items-center">
            <span className="text-[9px] font-black text-[#52b788] uppercase tracking-widest mb-1">
              Current Power
            </span>
    ...
    <div className="relative flex flex-row items-end justify-center gap-2 sm:gap-10 lg:gap-16 mt-8 pt-12 sm:pt-24 pb-16 px-4">
      ...
      >
        <motion.div
          ...
          style={{
            background: 'radial-gradient(circle, #E8A838 0%, transparent 65%)',
            filter: 'blur(70px)',
          }}
        />
      </div>
...
function RankItem({
  ...
  return (
    <motion.div
      ...
      onClick={() => onSelect(user)}
      whileHover={!isTouchDevice() ? { x: 4, scale: isTop3 ? 1.01 : 1 } : undefined}
      ...
      <div className="relative z-10 flex items-center gap-4 w-full">
        ...
        >
          {user.effectEmoji && (
            <motion.div
              ...
              style={{
                background: `radial-gradient(circle, ${EFFECT_AURA_COLORS[user.effectEmoji] || 'rgba(250,204,21,0.4)'} 0%, transparent 70%)`,
              }}
        ...
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          <span
            ...
            style={{ color: '#52b788', letterSpacing: '-0.03em' }}
          >
            {user.score.toLocaleString()}
            {user.scoreLabel}
          </span>
    ...
    >
      ...
      <div className="flex-1 flex items-center gap-10 z-10 w-full">
        <div className="text-center min-w-[100px]">
          <span className="block text-emerald-300/50 text-xs font-bold uppercase tracking-widest mb-2">
            나의 현재 순위
          </span>
          <div className="relative">
            ...
            {data.rank !== '-' && (
              <span className="text-lg sm:text-xl font-bold text-[#E8A838] ml-0.5">위</span>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-emerald-500/30 bg-white flex items-center justify-center text-2xl">
                {(() => {
                  const avatarUrl = generateRankingAvatar(
                    data.userId || 0,
                    data.rank === '-' ? 99 : data.rank,
                    data.equippedAvatarUrl,
                  );
...
export function RankingPage({ openAuth }: { openAuth: (mode: 'login' | 'signup') => void }) {
  ...
  const users: RankUser[] =
    isDataValid && Array.isArray(data.topRankers)
      ? data.topRankers
          ...
          .map((r) => ({
            ...
            titleBg:
              Number(r.rank) === 1
                ...
                : Number(r.rank) === 2
                  ? 'rgba(176,184,180,0.12)'
                  : Number(r.rank) === 3
                    ? 'rgba(205,124,74,0.12)'
                    : 'rgba(82,183,136,0.1)',
            ...
            effectEmoji:
              (r.equippedItems || []).find((item) => item.type === 'EFFECT')?.icon ?? undefined,
          }))
      : [];
    ...
    <div className="min-h-screen" style={{ background: '#F8FAF9' }}>
      ...
      <div className="relative overflow-hidden" style={{ background: '#F8FAF9' }}>
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse 60% 40% at 50% 0%, rgba(232,168,56,0.12) 0%, transparent 70%)',
          }}
        />
        ...
        <section className="relative z-10 pt-28 sm:pt-32 pb-24 sm:pb-40 px-4 sm:px-6">
          <div className="max-w-4xl mx-auto">
              ...
              >
                {tab === 'local' && !regionName ? (
                  <motion.div
                    ...
                    style={{
                      background:
                        'linear-gradient(135deg, rgba(27,67,50,0.05) 0%, rgba(232,168,56,0.08) 100%)',
                  ...
                  >
                    ...
                    {/* 타이틀 */}
                    <h3
                      className="text-xl sm:text-2xl font-black mb-3 relative z-10 leading-tight text-center"
                      style={{
                        background:
                          'linear-gradient(135deg, #1B4332 0%, #2D6A4F 50%, #E8A838 100%)',
                        WebkitBackgroundClip: 'text',
                        WebkitTextFillColor: 'transparent',
                        backgroundClip: 'text',
                      }}
                    ...
                    <div className="mt-8 p-4 rounded-2xl bg-white/60 backdrop-blur-sm border border-gray-100 relative z-10">
                      <p className="text-xs text-gray-500 text-center">
                        💡 <span className="font-bold">TIP:</span> 지도에서 가까운 화장실을 찾아
                        방문 인증을 완료하면
                        <br className="hidden sm:block" />
                  ...
                  <div className="flex flex-col items-center justify-center py-20 px-6 bg-white/50 rounded-3xl border border-gray-100">
                    ...
                    <p className="text-sm text-gray-500 text-center">첫 번째 랭커가 되어보세요!</p>
                  </div>
                ) : (
              ...
              {[
                {
                  label: '활성 사용자',
                  value:
                    data && typeof data.activeUserCount === 'number'
                ...
                },
                {
                  label: '상위권 도전',
                  value:
                    myRankData && typeof myRankData.rank === 'number' && myRankData.rank > 10
                ...
                >
                  ...
                  >
                    {s.value}
                    <span className="text-sm sm:text-base font-bold ml-0.5">{s.unit}</span>
                  </p>
                  <p className="text-sm mt-1.5 font-bold" style={{ color: 'rgba(0,0,0,0.4)' }}>
                    {s.label}
                  </p>
                </motion.div>
              ))}
            ...
            <div className="mt-4 mb-4">{myRankData && <MyRankBar data={myRankData} />}</div>
          </div>
        </section>
        ...
        <section className="pt-16 pb-40 px-6">
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center justify-between mb-10">
              <h2
                className="font-black text-3xl text-[#1A2B27]"
