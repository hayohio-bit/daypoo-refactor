  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <motion.div
          animate={shake && errors.email ? { x: [-10, 10, -10, 10, 0] } : {}}
        ...
        <motion.div
          animate={shake && errors.password ? { x: [-10, 10, -10, 10, 0] } : {}}
          transition={{ duration: 0.3 }}
        >
          <InputField
            label="비밀번호"
        ...
        <div className="flex items-center justify-between flex-wrap gap-y-2 -mt-1">
          <div className="flex items-center gap-3 flex-wrap">
            <label className="flex items-center gap-1.5 cursor-pointer select-none whitespace-nowrap">
              ...
              />
              <span className="text-[11px] xs:text-xs" style={{ color: 'rgba(26,43,39,0.5)' }}>
                아이디 기억하기
              </span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer select-none whitespace-nowrap">
          ...
          <Link
            to="/forgot-password"
            onClick={onClose}
            className="text-[11px] xs:text-xs transition-colors hover:text-[#1A2B27] whitespace-nowrap"
            style={{ color: 'rgba(26,43,39,0.4)' }}
          >
            비밀번호를 잊으셨나요?
          </Link>
        </div>
      ...
      <p className="text-center mt-5 text-sm" style={{ color: 'rgba(26,43,39,0.4)' }}>
        ...
        <button
          onClick={onSwitch}
          className="font-bold transition-colors hover:text-[#2d6a4f]"
          style={{ color: '#1B4332' }}
        >
          회원가입
        </button>
