...
function BirthDropdowns({ year, month, day, onYearChange, onMonthChange, onDayChange }: any) {
  ...
  const daysCount =
    year && month ? new Date(Number.parseInt(year), Number.parseInt(month), 0).getDate() : 31;
  ...
  const SelectWrapper = ({ value, onChange, options, placeholder, suffix }: any) => (
    <div className="relative group flex-1">
      ...
      <div
        className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none transition-colors group-hover:text-[#E8A838]"
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          {checks.map((c) => (
            ...
            >
              <Check size={9} strokeWidth={c.ok ? 3 : 1.5} />
              {c.label}
            </span>
          ))}
        </div>
  ...
  return (
    <div className="text-left">
            ...
            {step === 2 && (
              <div className="flex flex-col gap-3">
                ...
                <TermsCheck
                  ...
                  onChange={(v) => {
                    setAgreeService(v);
                    syncAll(v, agreePrivacy, agreeMarketing);
                  }}
                >
                  서비스 이용약관
                </TermsCheck>
                <TermsCheck
                  ...
                  onChange={(v) => {
                    setAgreePrivacy(v);
                    syncAll(agreeService, v, agreeMarketing);
                  }}
                >
                  개인정보처리방침
                </TermsCheck>
        ...
        <div className="flex gap-2 mt-5">
          {step > 0 && (
            <button
              ...
              className="w-11 h-11 flex items-center justify-center rounded-xl border border-gray-100"
            >
      ...
      <p className="text-center mt-4 text-sm" style={{ color: 'rgba(26,43,39,0.4)' }}>
        이미 계정이 있으신가요?{' '}
        <button onClick={onSwitch} className="font-bold text-[#1B4332]">
          로그인
        </button>
      </p>
    </div>
