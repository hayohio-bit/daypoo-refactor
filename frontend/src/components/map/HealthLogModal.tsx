...
const BRISTOL_DETAILS = [
  {
    ...
    desc: '분리된 딱딱한 덩어리',
    fullDesc:
      '수분이 극도로 부족한 상태로, 심한 변비를 의미합니다. 장내 체류 시간이 길어져 돌멩이처럼 딱딱한 배변이 발생합니다.',
    status: '심한 변비',
    color: '#8d6e63',
  },
  {
    ...
    desc: '덩어리가 뭉친 소시지 모양',
    fullDesc:
      '수분이 다소 부족하여 바게트처럼 단단하게 덩어리들이 뭉치 있습니다. 가벼운 변비가 의심되며 수분 섭취가 권장됩니다.',
    status: '변비 경향',
    color: '#a1887f',
  },
  {
    ...
    desc: '표면에 균열이 있는 소시지',
    fullDesc:
      '정상 범주에 속하지만 땅콩 껍질처럼 다소 표면에 균열이 있고 단단할 수 있습니다. 규칙적인 생활과 섬유질 섭취가 도움이 됩니다.',
    status: '정상 (다소 단단)',
    color: '#bcaaa4',
  },
  {
    ...
    emoji: '🍌',
    label: '매끄러운 바나나',
    desc: '부드럽고 매끄러운 바나나 모양',
    fullDesc:
      '가장 이상적인 쾌변 상태입니다! 뱀처럼 부드럽고 매끈하며 적절한 탄력을 가진 최고의 컨디션입니다.',
    status: '매우 양호',
    color: '#8BC34A',
  },
  {
    ...
    desc: '부드러운 덩어리, 경계 뚜렷',
    fullDesc:
      '부드러운 알맹이 형태로 경계가 뚜렷합니다. 전반적으로 양호한 상태이며 원활한 소화가 이루어지고 있습니다.',
    status: '양호 (약간 무름)',
    color: '#4caf50',
  },
  {
    ...
    emoji: '🍮',
    label: '흐물흐물한 푸딩형',
    desc: '경계가 불분명하고 무른 모양',
    fullDesc:
      '푸딩처럼 형태가 불분명하고 매우 무른 상태입니다. 설사로 진행되기 직전이므로 자극적인 음식 섭취를 피하고 장 휴식을 권장합니다.',
    status: '경미한 설사',
    color: '#ff9800',
  },
  {
    ...
    desc: '덩어리가 전혀 없는 액체',
    fullDesc:
      '심한 설사 상태입니다. 수분 손실이 많으므로 전해질 보충이 시급하며 탈수에 강한 주의가 필요합니다.',
    status: '심한 설사',
    color: '#f44336',
  },
];
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center justify-center py-12 px-8 space-y-8 min-h-[460px]">
            ...
            <div className="text-center space-y-3">
              ...
              <p className="text-sm text-[#7a9e8a] font-medium leading-relaxed">
                성공적으로 보관되었습니다.
                <br />
                변화하는 컨디션 데이터를 리포트로 확인해보세요.
              </p>
            </div>
            <div className="w-full space-y-3 pt-6">
              <WaveButtonComponent
                ...
                onClick={() => {
                  window.location.href = '/mypage?tab=report';
                }}
              ...
              <button
                onClick={onClose}
                className="w-full py-3 text-[#b5c9bc] font-bold text-sm hover:text-[#7a9e8a] transition-colors"
              >
                닫기
              </button>
            </div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                        ...
                        {!isCameraActive && !capturedImage && (
                          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                            ...
                            <WaveButtonComponent
                              ...
                              size="md"
                              className="shadow-xl"
                              icon={<Camera size={20} className="animate-pulse" />}
                            >
                              카메라 실행하기
                            </WaveButtonComponent>
                            <p className="mt-3 text-[10px] text-[#7a9e8a]/70 font-bold tracking-tight">
                              촬영은 선택사항이며, 언제든 직접 입력 가능합니다.
                            </p>
                          </div>
                        )}
                        {isCameraActive && (
                  ...
                  {step === 1 && (
                    <div className="flex flex-col md:flex-row gap-6">
                      <div className="flex-1 space-y-4">
                        <div>
                          <p className="font-black text-2xl text-[#1a2b22] md:text-xl">모양 선택</p>
                          <p className="text-base text-[#7a9e8a] mt-0.5 md:text-sm">
                            상태에 가장 가까운 모양을 선택해주세요.
                          </p>
                        </div>
                        <div className="grid gap-1.5">
                          {BRISTOL_DETAILS.map((b) => (
                            <button
                              ...
                              onMouseEnter={() => setHoveredBristol(b.type)}
                              onMouseLeave={() => setHoveredBristol(null)}
                            ...
                            >
                              ...
                              <div className="text-left flex-1">
                                <p
                                  className={`text-base font-bold md:text-sm ${bristolType === b.type ? 'text-[#1B4332]' : 'text-[#1a2b22]'}`}
                                >
                                  {b.type}단계 · {b.label}
                                </p>
                              </div>
                              {bristolType === b.type && (
                                <Check size={22} className="text-[#1B4332]" />
                              )}
                            </button>
                          ))}
                        </div>
                      ...
                      <div className="hidden md:flex flex-col w-[280px] bg-[#1a2b27] rounded-[28px] p-6 shadow-2xl border border-white/10 shrink-0 min-h-[380px]">
                        <AnimatePresence mode="wait">
                          {hoveredBristol || bristolType ? (
                            <m.div
                              key={hoveredBristol || bristolType}
                              ...
                              className="space-y-6"
                            >
                              <div className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center text-5xl shadow-inner">
                                {
                                  BRISTOL_DETAILS.find(
                                    (b) => b.type === (hoveredBristol || bristolType),
                                  )?.emoji
                                }
                              </div>
                              <div className="space-y-2">
                                <span
                                  className="inline-block px-3 py-1 rounded-full bg-white/10 text-[10px] font-black uppercase tracking-tighter"
                                  style={{
                                    color: BRISTOL_DETAILS.find(
                                      (b) => b.type === (hoveredBristol || bristolType),
                                    )?.color,
                                  }}
                                >
                              ...
                              <p className="text-[#7a9e8a] text-sm font-medium leading-relaxed">
                                {
                                  BRISTOL_DETAILS.find(
                                    (b) => b.type === (hoveredBristol || bristolType),
                                  )?.fullDesc
                              ...
                              <div className="pt-4 border-t border-white/5">
                                ...
                                <div className="mt-2 flex items-center gap-2 text-white/80 text-xs font-bold italic">
                            ...
                            >
                              <div className="w-20 h-20 rounded-full bg-white/5 flex items-center justify-center">
                                <Sparkles size={32} className="text-[#3b5b4a]" />
                              </div>
                              <div className="space-y-1">
                                <p className="text-white font-bold text-base">상태별 가이드</p>
                                <p className="text-[#7a9e8a] text-xs leading-relaxed px-4">
                                  모양을 선택하거나 마우스를 올려
                                  <br />
                                  상세 정보를 확인해보세요.
                                </p>
                  ...
                  {step === 2 && (
                    <div className="space-y-6">
                      <div>
                        <p className="font-black text-2xl text-[#1a2b22] md:text-xl">
                          어떤 색에 가깝나요?
                        </p>
                        <p className="text-base text-[#7a9e8a] mt-1 md:text-sm">
                          가장 일치하는 색상을 골라주세요.
                        </p>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {(
                          Object.entries(POOP_COLORS) as [
                            PoopColor,
                          ...
                          >
                            <div className="relative">
                              <AnimatePresence>
                                {color === key && (
                                  <m.div
                                    ...
                                    animate={{ scale: [1, 1.3, 1], opacity: 0.3 }}
                              ...
                              <div className="w-16 h-16 md:w-14 md:h-14 relative z-10">
                                <m.div
                                  animate={
                                    color === key || hoveredColor === key
                                      ? {
                                          scale: [1, 1.3, 1.3, 1, 1],
                                          rotate: [0, 0, 270, 270, 0],
                                          borderRadius: ['20%', '20%', '50%', '50%', '20%'],
                                        }
                                      : { scale: 1, rotate: 0, borderRadius: '50%' }
                                  }
                                  transition={
                                    color === key || hoveredColor === key
                                      ? {
                            ...
                            <span
                              className={`text-base font-bold transition-colors md:text-sm ${color === key ? 'text-[#1B4332]' : 'text-[#1a2b22]'}`}
                            >
                              {val.label}
                            </span>
                            {color === key && (
                              <m.div
                                initial={{ scale: 0 }}
                                animate={{ scale: 1 }}
                                className="absolute top-3 right-3 w-6 h-6 bg-[#1B4332] rounded-full flex items-center justify-center text-white"
                              >
                                <Check size={14} />
                              </m.div>
                            )}
                          ...
                          {CONDITION_TAGS.map((tag) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-5 py-2.5 rounded-full border-2 font-bold text-base md:text-sm transition-all duration-300 ${
                                  isSelected
            ...
            <div className="px-6 py-6 bg-[#fcfdfc] border-t border-[#eef5f0] flex gap-3">
              {step > 0 && (
                <button
                  ...
                  className="flex items-center justify-center w-14 h-14 rounded-2xl border-2 border-[#eef5f0] text-[#7a9e8a] hover:bg-[#f4faf6] transition-colors"
                >
                  <ChevronLeft size={24} />
                </button>
              )}
                ...
                disabled={
                  step === 1
                    ...
                    : step === 2
                      ? !color
                      : step === 3
                        ? conditions.length === 0 || foods.length === 0
                        : false
                }
                variant="primary"
                size="lg"
                className="flex-1 shadow-lg overflow-hidden group"
              >
                <div className="relative h-6 flex items-center justify-center min-w-[120px]">
                  <AnimatePresence mode="wait">
                    {isNextHovered &&
                    step > 0 &&
                    (step === 1 ? bristolType : step === 2 ? color : true) ? (
                      <m.div
                        key="check"
